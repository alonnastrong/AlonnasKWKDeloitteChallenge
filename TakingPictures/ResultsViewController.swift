...
//  TakingPictures
//
//  Created by Alonna Strong on 2025-01-01.
//
import UIKit
import AVFoundation
import Vision
...
class ResultsViewController: UIViewController, AVCaptureVideoDataOutputSampleBufferDelegate {
    ...
    private func setupUI() {
        ...
        resultsLabel.backgroundColor = .systemPink
    ...
    @objc func handleCapture() {
        ...
        let alertController = UIAlertController(
            ...
            message: "Detected Ingredient: \(ingredient). Is this correct?",
        ...
        alertController.addAction(UIAlertAction(title: "Yes", style: .default, handler: { [weak self] _ in
            ...
            if ingredient.lowercased() == "onion" {
                self.transitionToOnionViewController()
            } else {
    ...
    func transitionToOnionViewController() {
        let onionViewController = OnionViewController() // Ensure OnionViewController exists and is a UIViewController
        onionViewController.modalPresentationStyle = UIModalPresentationStyle.fullScreen
        present(onionViewController, animated: true, completion: nil)
    }
